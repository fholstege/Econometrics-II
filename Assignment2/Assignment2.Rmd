...
stargazer(lm_pooled_no_ab, lm_pooled_ab,
          ...
          se = list(vcovHC(lm_pooled_no_ab, cluster = "group"), vcovHC(lm_pooled_no_ab, cluster="group")),
...
stargazer(lm_inter, lm_white, lm_black,
          keep.stat=c("n","adj.rsq"),
          se = list(vcovHC(lm_inter, cluster="group"), vcovHC(lm_white, cluster="group"), vcovHC(lm_black, cluster="group")),
...
The table below shows the schooling coefficient (the average effect for both groups), and the interaction term between schooling and ethnicity, for the fixed effects model. This latter effect should be interpret as follows: it measures the effect of schooling on earnings for black individuals (when the "ethnicity" variable is 1). On average, for both groups, one year of schooling leads to an 5.3\% increase in earnings. For black individuals, this effect is 6.2\% lower, and statistically significant. One might wonder what the intuition is behind this result?. It basically says; controlling for several other variables (age, region, etc. ) and fixed effects of these variables over time (e.g. an individuals average level of schooling), one extra year of schooling has significantly less returns for black individuals than right individuals. 
