...
To check validity of the instruments, we look at correlation of the errors terms from OLS regression and two instruments: distance and regional subsidies. For a joint instruments we use Sargan test.

To add to this intuition, regress the log of earnings on schooling without instruments, and with three different instruments: the subsidy variable, the distance variable, and both of them. For each, we conduct the Hausman test, and where possible the Sargan test. The results are shown below:
...
\begin{table}[!htbp] \centering 
\small
  \caption{The results per set of instruments} 
  \label{} 
...
\cline{2-5} 
\\[-1.8ex] & \multicolumn{4}{c}{logWage} \\ 
 & \textit{} & \multicolumn{3}{c}{\textit{variable}} \\ 
\\[-1.8ex] & No Instrument & Subsidy Instrument & Distance Instrument & Subsidy and Distance Instrument\\ 
\hline \\[-1.8ex] 
 schooling & 0.101 & 0.730$^{***}$ & 0.100 & 0.658$^{***}$ \\ 
  & (0.073) & (0.262) & (0.626) & (0.245) \\ 
  & & & & \\ 
 Constant & 5.778$^{***}$ & 1.599 & 5.789 & 2.078 \\ 
  & (0.512) & (1.753) & (4.163) & (1.639) \\ 
  & & & & \\ 
\hline \\[-1.8ex] 
Observations & 416 & 416 & 416 & 416 \\ 
R$^{2}$ & 0.005 & $-$0.176 & 0.005 & $-$0.137 \\ 
Hausman (p-value) & -&0.006 & 0.998 & 0.010 \\
Sargan (p-value) & - &- & - & 0.37 \\
\hline 
\hline \\[-1.8ex] 
